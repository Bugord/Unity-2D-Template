namespace Undebugger.Services.Trigger
{
    public static class CommonTriggers
    {
        ...
        public static MenuTriggerAction ToggleMenuWithF2Key(bool isOpen)
        {
#if ENABLE_INPUT_SYSTEM
            return
                UnityEngine.InputSystem.Keyboard.current.f2Key.wasReleasedThisFrame ?
                    isOpen ?
                    MenuTriggerAction.Close :
                    MenuTriggerAction.Open :
                MenuTriggerAction.None;
#else
            return UnityEngine.Input.GetKeyUp(UnityEngine.KeyCode.F2) ? (isOpen ? MenuTriggerAction.Close : MenuTriggerAction.Open) : MenuTriggerAction.None;
#endif
        }
